...
int main(int argc, char* argv[])
{
	...
	while (!glfwWindowShouldClose(g_Window))
	{
		...
		// render camera-spotlight shadow map before the lit pass
		{
			// retrieve camera for spotlight position/direction
			glm::vec3 camPos = g_ViewManager->GetCameraPosition();
			glm::vec3 camDir = g_ViewManager->GetCameraFront();
			g_SceneManager->RenderShadowMap(camPos, camDir);
			// re-bind main shader for subsequent uniform updates and drawing
			g_ShaderManager->use();
		}
...
bool InitializeGLEW()
{
	...
	// enable experimental features for broader driver compatibility (Windows)
